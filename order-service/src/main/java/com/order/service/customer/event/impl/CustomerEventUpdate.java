...
@Component
@Slf4j
public class CustomerEventUpdate implements CustomerEventChain {

    private final CustomerRepository customerRepository;

    public CustomerEventUpdate(CustomerRepository customerRepository) {
        this.customerRepository = customerRepository;
    }

    @Override
    public void execute(CustomerEventRequest customerEventRequest) {
        log.info("Executing update event");
    ...
    public boolean canHandle(CustomerEventRequest customerEventRequest) {
        return CustomerEventType.UPDATE.equals(customerEventRequest.getCustomerEventType());
    }
}
